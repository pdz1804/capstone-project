...
\subsection{Detailed Component Architecture of Multimodal RAG System}
...
\begin{enumerate}
    ...
    \item \textbf{LLM Generation}: The aggregated context (text chunks + rendered page images) is passed to a Multimodal Large Language Model. The system is provider-agnostic, supporting OpenAI GPT-4o, Google Gemini, Anthropic Claude, and local models via Ollama (Llama, Mistral, Qwen).
...
The complete Multimodal RAG architecture integrates all three layers seamlessly. The system provides a FastAPI-based REST interface with asynchronous I/O support, exposing endpoints for document upload (\texttt{/api/upload}), processing (\texttt{/api/process}), querying (\texttt{/api/query}), and status monitoring (\texttt{/api/status}).
