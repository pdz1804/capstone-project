...
\begin{figure}[H]
    ...
    \caption{Data Normalization Flowchart: From Raw Input to RAG-Ready Artifacts.}
...
\begin{itemize}
    \item \textbf{Filename Sanitization}: To prevent filesystem collisions and encoding errors, filenames are normalized to ASCII characters, and duplicate uploads are handled via an incremental suffix strategy (e.g., \texttt{lecture\_notes.pdf} $\rightarrow$ \texttt{lecture\_notes\_1.pdf}).
    \item \textbf{MIME Type Validation}: The system acts as a gatekeeper, filtering files based on magic numbers to ensure only supported formats proceed to the processing stage.
\end{itemize}
...
\begin{itemize}
    \item \textbf{Tooling}: The system utilizes \textbf{LibreOffice} in headless mode (server-side).
    \item \textbf{Transformation}: These documents are converted into standard PDF format. This ensures that layout, fonts, and slide structures are preserved visually for the image retrieval engine.
\end{itemize}
...
\begin{itemize}
    \item \textbf{Audio Extraction}: \textbf{MoviePy} is employed to separate audio tracks from video containers.
    \item \textbf{Transcription}: The \textbf{OpenAI Whisper} model (Automatic Speech Recognition) processes the audio stream to generate high-fidelity transcripts. These transcripts are saved as Markdown text files, time-stamped to allow for future temporal retrieval.
\end{itemize}
...
\begin{itemize}
    \item \textbf{Tooling}: The pipeline integrates \textbf{Docling}, a specialized document parsing library.
    \item \textbf{Capability}: Docling analyzes the document structure to distinguish between headers, paragraphs, and tables.
    \item \textbf{Output}: It outputs structured \textbf{Markdown} that preserves table boundaries (CSV-style) and document hierarchy. This allows the text chunker to respect semantic boundaries rather than cutting sentences mid-table.
\end{itemize}
