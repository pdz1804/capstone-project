...
class DocxPreprocessor:
    ...
    def _rewrite_image_paths(tree: List[Dict], images_dir: Path) -> None:
        """Replace temp image paths in the tree with their copied location."""
        for node in tree:
            ...
            children = node.get("children", [])
            if children:
                DocxPreprocessor._rewrite_image_paths(children, images_dir)
