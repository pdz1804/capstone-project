...
from app.api.schemas import RemoveFromIndexRequest
...
async def process(
    ...
    try:
        stats = run_processing(user_id=user_id, force=force)
...
async def index_text(
    ...
    cfg = merged_runtime_settings()
    try:
        out = IndexingService(cfg, user_id=user_id).index_text(force=force)
...
async def index_image(
    ...
    cfg = merged_runtime_settings()
    try:
        out = IndexingService(cfg, user_id=user_id).index_images(force=force)
        if out.get("status") == "failed":
            raise HTTPException(status_code=500, detail=out.get("error"))
        return {"status": "completed", "results": out}
    except HTTPException:
