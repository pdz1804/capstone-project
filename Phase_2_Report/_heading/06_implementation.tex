...
\begin{figure}[!ht]
    \centering
    \includegraphics[width=1\textwidth]{{img/pdz/Report 252 Diagram-Complete Pipeline Architecture.drawio.png}}
...
\begin{figure}[!ht]
    \centering
    \includegraphics[width=1\textwidth]{{img/pdz/Report 252 Diagram-File Type Routing.drawio.png}}
...
PPTX / PPT & File extension & LibreOffice & \checkmark & â€” \\
HTML & MIME type & BeautifulSoup + Docling & \checkmark & â€” \\
Markdown & Extension & Copy as-is & â€” & â€” \\
TXT & Extension & Copy as-is & â€” & â€” \\
CSV & Extension & Pandas & â€” & â€” \\
Images (JPG, PNG) & Extension & img2pdf & \checkmark & â€” \\
Video (MP4, AVI) & Extension & MoviePy (audio extraction) & â€” & â€” (â†’ Stage 2) \\
Audio (WAV, MP3) & Extension & Pass-through & â€” & â€” (â†’ Stage 2) \\
\bottomrule
\end{tabular}%
}
\end{table}
...
\begin{figure}[!ht]
    \centering
    \includegraphics[width=0.75\textwidth]{{img/pdz/Report 252 Diagram-Media Processing Flow.drawio.png}}
...
\begin{figure}[!ht]
    \centering
    \includegraphics[width=1\textwidth]{{img/pdz/Report 252 Diagram-DocumentProcessorV2 Unified Router.drawio.png}}
...
% BM25 Index (Text) & Inverted index (term â†’ docs) & Pickle file (\texttt{.pkl}) & Keyword matching, TF-IDF scoring \\
...
% \begin{enumerate}
%     \item Chunks from Stage 4 are encoded by SBERT (all-MiniLM-L6-v2) â†’ 384-dimensional vectors
...
% \toprule
% \textbf{Index Type} & \textbf{Data Structure} & \textbf{Storage Format} & \textbf{Query Mechanism} \\ \midrule
% BM25 Index (Text) & Inverted index (term â†’ docs) & Pickle file (\texttt{.pkl}) on S3 & Keyword matching, TF-IDF scoring \\
...
\begin{figure}[!ht]
    \centering
    \includegraphics[width=0.8\textwidth]{img/pdz/Report 252 Diagram-Search Flow.drawio.png}
    \caption[Search and Retrieval Pipeline Flow]{Search and Retrieval Pipeline Flow.}
    \label{fig:search_flow}
\end{figure}
...
\begin{figure}[!ht]
    ...
    \begin{verbatim}
    S3/
    â”œâ”€â”€ ai-service-originals-dev/         # Bucket for raw document ingestion
    â”‚   â””â”€â”€ users/{uid}/                  # Logical user-tenant isolation
    â”‚       â”œâ”€â”€ {doc_name}.pdf            # Original source document
    â”‚       â””â”€â”€ {doc_name}.pdf.metadata.json # Ingestion time & file provenance
    â””â”€â”€ ai-service-processed-dev/         # Bucket for pipeline artifacts
        â””â”€â”€ users/{uid}/
            â”œâ”€â”€ .processing_cache.json    # Idempotency and deduplication tracking
            â”œâ”€â”€ pipeline_stats.json       # Telemetry and processing performance
            â”œâ”€â”€ stage1_normalized/        # Standardized PDF & structured data
            â”‚   â”œâ”€â”€ normalized_pdfs/      # Unified format for visual processing
            â”‚   â”œâ”€â”€ excel_parsed/         # Structured tables from Office files
            â”‚   â”œâ”€â”€ original_files/       # Internal reference copy
            â”‚   â””â”€â”€ normalization_metadata/
            â”œâ”€â”€ stage2_media_processed/   # Extracted audiovisual artifacts
            â”‚   â”œâ”€â”€ extracted_audio/      # Raw audio for Whisper transcription
            â”‚   â”œâ”€â”€ extracted_frames/     # Visual frames for late-interaction RAG
            â”‚   â”œâ”€â”€ transcripts/          # ASR textual outputs
            â”‚   â””â”€â”€ media_metadata/       # Temporal feature mapping
            â”œâ”€â”€ stage3_document_processed/# Structural Markdown outputs
            â”‚   â”œâ”€â”€ {doc_name}/
            â”‚   â”‚   â”œâ”€â”€ {doc_name}.md     # Cleaned semantic Markdown
            â”‚   â”‚   â””â”€â”€ {doc_name}_metadata.json
            â”‚   â””â”€â”€ logs/                 # Granular stage-level logs
            â”‚       â”œâ”€â”€ batch_summary_*.json
            â”‚       â””â”€â”€ processing_*.log
            â”œâ”€â”€ stage4_rag_ready/         # Optimized artifacts for RAG engine
            â”‚   â”œâ”€â”€ {doc_name}/
            â”‚   â”‚   â”œâ”€â”€ {doc_name}.md     # Final semantic text
            â”‚   â”‚   â”œâ”€â”€ {doc_name}.pdf    # Optimized PDF for visual retrieval
            â”‚   â”‚   â””â”€â”€ docling_additional/
            â”‚   â””â”€â”€ consolidation_stats.json
            â””â”€â”€ retrieval/                # Secondary persistence for search indices
                â”œâ”€â”€ bm25_index.pkl        # Serialized sparse search index
                â””â”€â”€ documents.json.pkl    # Document lookup table
    \end{verbatim}
...
To manage long-running asynchronous tasksâ€”such as full-document re-indexing, multimodal extraction, or batch analysisâ€”the system utilizes an \textbf{Amazon DynamoDB} table (name: \texttt{bk\_mind\_app\_jobs}). This table acts as a task registry, tracking the lifecycle and progress of background operations to ensure system resilience and provide real-time status updates to the user interface.
...
The generative synthesis layer is powered by Large Language Models orchestrated via AWS Bedrock and OpenAI providers. The following hyperparameters are tuned per service to balance factual grounding with appropriate response variety. The model identifier is configurable via environment variable, supporting both Claude 3.5 Sonnet (via Bedrock) and GPT-4o (via OpenAI).

\begin{table}[!ht]
    \centering
    \caption{LLM Generation Hyperparameters by Service}
    \label{tab:llm_hyperparameters}
    \scalebox{0.9}{
    \begin{tabular}{lcccp{4cm}}
        \toprule
        \textbf{Service} & \textbf{Temperature} & \textbf{Top-P} & \textbf{Max Tokens} & \textbf{Generation Focus} \\
        \midrule
        Chat Assistant & 0.7 & 0.9 & 4096 & Conversational, Socratic dialogue \\
        Knowledge Explorer & 0.0 & 1.0 & 2000 & Deterministic, fact-driven \\
        Summary Generation & 0.2 & 1.0 & 3000 & Analytical, concise \\
        MCQ Generation & 0.2 & 1.0 & 3000 & Structural, objective \\
        Learning Roadmap & 0.2 & 1.0 & 3000 & Pedagogical, structured \\
        \bottomrule
    \end{tabular}
    }
\end{table}

Temperature controls creativity: factual services (Summary, MCQ, Roadmap) use 0.2 to minimize hallucinations; Knowledge Explorer uses 0.0 for maximum determinism; Chat Assistant uses 0.7 for engaging Socratic dialogue. Top-P is set between 0.9 and 1.0 to maintain high-probability semantic output while allowing linguistic variety in educational explanations.
% \section{Generative Configuration and Hyperparameters}
% 
% The generative synthesis layer is powered by Large Language Models (LLMs) orchestrated via the Bedrock and OpenAI providers. To ensure consistent output quality, factual grounding, and adherence to specific educational personas, the system employs a set of tuned hyperparameters. These parameters are dynamically adjusted based on the specific service being invoked to balance creative engagement with factual precision.
% 
% \begin{table}[!ht]
%     \centering
%     \caption{LLM Hyperparameters for Core System Services}
%     \label{tab:llm_hyperparameters}
%     \scalebox{0.9}{
%     \begin{tabular}{lp{4cm}cccp{4cm}}
%         \toprule
%         \textbf{Service} & \textbf{Primary Model} & \textbf{Temp} & \textbf{Top-P} & \textbf{Max Tokens} & \textbf{Persona Focus} \\
%         \midrule
%         \textbf{Chat Assistant} & Claude 3.5 Sonnet & 0.7 & 0.9 & 4096 & Conversational, Socratic \\
%         \textbf{Knowledge Explorer} & Claude 3.5 Sonnet & 0.0 & 1.0 & 2000 & Fact-driven, Direct \\
%         \textbf{Summary Gen} & Claude 3.5 Sonnet & 0.2 & 1.0 & 3000 & Analytical, Concise \\
%         \textbf{MCQ Generation} & Claude 3.5 Sonnet & 0.2 & 1.0 & 3000 & Structural, Objective \\
%         \textbf{Roadmap Gen} & Claude 3.5 Sonnet & 0.2 & 1.0 & 3000 & Pedagogical, Structured \\
%         \bottomrule
%     \end{tabular}
%     }
% \end{table}
% 
% The hyperparameter selection reflects a strategic trade-off between exploration and exploitation:
% \begin{itemize}
%     \item \textbf{Temperature}: Factual services (Summary, MCQ, Roadmap) use a conservative temperature of $0.2$ to minimize hallucinations, while the \textbf{Knowledge Explorer} utilizes a strictly deterministic setting ($0.0$) for maximum grounding. The \textbf{Chat Assistant} uses a higher value ($0.7$) to facilitate engaging, Socratic-style dialogue.
%     \item \textbf{Top-P (Nucleus Sampling)}: Set between $0.9$ and $1.0$ to ensure the model remains within a high-probability semantic manifold while allowing for sufficient linguistic variety in educational explanations.
%     \item \textbf{Max Tokens}: Calibrated for each task, with $3,000$ tokens allocated for comprehensive document-level insights and $4,096$ for extended conversational contexts.
% \end{itemize}
...
\begin{table}[!ht]
    ...
    \begin{tabular}{llp{7cm}}
        ...
        \midrule
        Run Pipeline & \texttt{POST /api/process} & Orchestrates the 7-stage multimodal extraction and normalization with conditional routing. \\
        \midrule
...
The API service layer described above provides the complete interface through which frontend clients interact with the backend processing pipelines. To operate reliably at production scale, this API and all supporting services must be packaged, deployed, and managed within a containerized infrastructure with automated scaling and monitoring. The following section describes the deployment architecture and orchestration strategy.
...
\begin{enumerate}
    ...
    \item \textbf{IP Reputation List (Priority 4):} Leverages the Amazon IP Reputation Listâ€”a continuously updated database of IPs known for malicious activity including botnet C\&C servers, malware distribution endpoints, and credential stuffing sources. Updated daily with false positive rate $<0.1\%$.
\end{enumerate}

\textbf{Capacity and Performance}

The WAF uses a consumption model based on Web ACL Capacity Units (WCUs). The current configuration consumes 1,129 WCUs out of a maximum capacity of 5,000 WCUs, representing 22.6\% utilization. This leaves significant headroom (3,871 WCUs) for future security enhancements. Performance testing shows that WAF processing adds only 1-5 milliseconds per requestâ€”less than 0.1\% of typical backend processing time (100-500ms) and imperceptible to end users.
