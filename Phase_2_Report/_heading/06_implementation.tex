...
Manages the initial ingestion of raw document assets into the system's storage backend (local filesystem or Amazon S3). This service supports high-concurrency multi-file uploads and ensures that all assets are logically partitioned by user identity (see Appendix \ref{chap:interface}, Figure \ref{fig:ui_upload}).
...
Initiates the 5-stage multimodal extraction and normalization pipeline for a specific set of assets (see Appendix \ref{chap:interface}, Figure \ref{fig:ui_pipeline}). 
...
Synchronizes extracted artifacts into searchable vector and keyword indexes. This service manages both the Qdrant visual/text collections and the BM25 sparse index (see Appendix \ref{chap:interface}, Figure \ref{fig:ui_index}).
...
Provides a high-fidelity search interface for granular document exploration, supporting hybrid retrieval (BM25 + Dense) and visual context rendering using multimodal embeddings (see Appendix \ref{chap:interface}, Figure \ref{fig:ui_explorer}).
...
Generates context-aware abstracts and targeted summaries by synthesizing the structured outputs (see Appendix \ref{chap:interface}, Figure \ref{fig:ui_summary}). This service utilizes the high-fidelity Markdown context of a specific document to generate synthesis while respecting thematic focus and stylistic constraints.
...
Synthesizes a structured educational progression and roadmap based on specific document contexts and user goals (see Appendix \ref{chap:interface}, Figure \ref{fig:ui_roadmap}). This pedagogical service leverages the LLM's reasoning capabilities to formulate a logical sequence of prerequisite and core topics.
...
Synthesizes dynamic multiple-choice quizzes and rationales based on specific document contexts (see Appendix \ref{chap:interface}, Figure \ref{fig:ui_mcq}). This service is designed for automated knowledge validation, supporting various difficulty levels and stylistic orientations.
...
The primary RAG orchestration endpoint for streaming conversational interaction and long-term memory management (see Appendix \ref{chap:interface}, Figure \ref{fig:ui_chat}). This service utilizes DynamoDB for session state and prompt injection for personalized personalization.
...
Logs user sentiment and enables asynchronous AI-driven quality analysis of RAG performance (see Appendix \ref{chap:interface}, Figure \ref{fig:ui_feedback}). This service captures both structured sentiment (votes) and unstructured qualitative data to drive system improvements.
