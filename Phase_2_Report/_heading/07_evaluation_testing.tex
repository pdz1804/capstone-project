...
\begin{table}[H]
    ...
    \scalebox{0.82}{
    \begin{tabular}{p{3.0cm}p{3.3cm}p{4.2cm}p{3.0cm}p{3.0cm}}
        \toprule
        ...
        Retrieval PDF corpus & PDF QA/RAG retrieval evaluation & Local corpus: \texttt{backend/input/eval-input} & 100 PDF candidates; 43 selected-section files; 347 visual pages & Selected files avg 1,927.2 KB; range 65.5--16,805.9 KB \\
        \bottomrule
    \end{tabular}
    }
\end{table}
...
Office-based evaluation uses a format-specific unit definition because office files do not share a single page model: DOCX uses pages from document properties, PPTX uses slides, and XLSX uses sheets. Table~\ref{tab:office_dataset_scale} reports the dataset scale used for the Office-based parsing evaluations.
...
\begin{table}[H]
    ...
    \begin{tabular}{lrrrrr}
        \toprule
        \textbf{Dataset Slice} & \textbf{Files} & \textbf{Avg Size (KB)} & \textbf{Median Size (KB)} & \textbf{Avg Unit} & \textbf{Unit Range} \\
        \midrule
        OfficeDocBench & 43 & 31.4 & 23.0 & 1.61 & 1--11 \\
        Pandoc DOCX & 72 & 20.2 & 14.9 & 1.14 pages & 1--7 \\
        Pandoc PPTX & 19 & 33.0 & 29.1 & 2.47 slides & 1--6 \\
        DECO XLSX & 50 & 242.1 & 71.4 & 10.46 sheets & 4--50 \\
        \bottomrule
    \end{tabular}
    }
\end{table}
...
\begin{table}[H]
    ...
    \scalebox{0.84}{
    \begin{tabular}{p{3.2cm}p{5.2cm}p{4.6cm}p{3.2cm}}
        \toprule
...
\begin{table}[H]
    \centering
    \caption{Visual Retrieval Performance on PDF Dataset (n = 380)}
