...
First, the team learned how difficult heterogeneous educational data can be in practice. Lecture materials contain slides, scanned pages, diagrams, tables, speech, timestamps, and course-specific terminology. This required the team to combine OCR, ASR, document parsing, chunking, metadata extraction, and multimodal indexing into a unified pipeline rather than treating each file type as an isolated case.

Second, the team learned that retrieval quality depends on system design as much as on model accuracy. Experiments with sparse, dense, hybrid, and visual retrieval showed that no single retrieval method is sufficient for educational content. BM25 remains important for exact technical terms, dense embeddings improve semantic matching, and visual retrieval is necessary for diagrams and layout-heavy materials. The project therefore taught the team to evaluate retrieval as an end-to-end engineering problem involving parsing quality, chunk granularity, metadata, ranking, and generation grounding.
