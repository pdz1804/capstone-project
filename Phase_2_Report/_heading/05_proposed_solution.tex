...
The BK-MInD system architecture, illustrated in Figure~\ref{fig:high_level_arch}, is organized into four interconnected layers that span from the user interface to the underlying cloud infrastructure. This architecture is designed to achieve three strategic objectives: (1) \textit{multimodal data ingestion} from diverse educational materials, (2) \textit{asynchronous processing} to support concurrent user operations without bottlenecks, and (3) \textit{production-grade security and scalability} on AWS infrastructure.

\subsubsection{Layer 1: Interface Layer (Frontend)}

The \textbf{Interface Layer} comprises the React-based frontend, which provides dedicated views for document upload, pipeline monitoring, knowledge exploration, learning roadmap generation, quiz creation, and an interactive chat assistant. All user interactions are routed through a single-page application that communicates with the backend exclusively via REST API calls. The frontend supports real-time status updates through polling mechanisms and streaming responses, enabling students and instructors to observe processing progress and retrieve answers with minimal perceived latency.

\subsubsection{Layer 2: API and Orchestration Layer}

The \textbf{API and Orchestration Layer} is implemented using FastAPI and acts as the central controller of the system. It exposes asynchronous endpoints for document ingestion (\texttt{/api/process}), index construction (\texttt{/api/index}), query processing (\texttt{/api/search}), and chat (\texttt{/api/chat}). Long-running operations are orchestrated by service classes (\texttt{ProcessingService}, \texttt{SearchOrchestrator}, \texttt{IndexingService}, \texttt{ChatService}), which delegate data access to repository classes that communicate with external services. Asynchronous jobs are managed through \textbf{Redis} (not SQS): job metadata is stored as Redis hash structures with per-user concurrency limits (3 concurrent jobs per user, 200 globally). This asynchronous design is critical to the system's ability to handle concurrent users: while one user's document undergoes the intensive 7-stage pipeline (which may take 26--28 seconds per document), other users' queries and uploads are processed independently without blocking.

\subsubsection{Layer 3: Data Processing and Indexing Layer}

The \textbf{Data Processing and Indexing Layer} implements the 7-stage conditional pipeline (detailed in Section~\ref{sec:pipeline_arch}) that transforms raw educational materials — PDFs, DOCX, PPTX, Excel spreadsheets, audio, and video — into RAG-ready artifacts. The pipeline follows three key design principles:
...
\subsubsection{Layer 4: Cloud Infrastructure Layer}

The \textbf{Cloud Infrastructure Layer} leverages Amazon Web Services for scalable deployment and production-grade availability. The architecture spans multiple availability zones in the us-west-2 (Oregon) region to ensure high availability:
...
\begin{itemize}
    ...
    \item \textbf{Async Job Management:} \textbf{Redis} is used as the primary job queue for asynchronous operations. Jobs are stored as Redis hash structures with metadata (status, user\_id, created\_at, results). Per-user concurrency limits (3 concurrent jobs) and global limits (200 concurrent jobs) are enforced via Redis sets. Job state transitions (accepted → running → completed/failed) are tracked in Redis with a default TTL of 3600 seconds for automatic cleanup. For critical deployments requiring durability, Amazon DynamoDB can serve as a fallback for job persistence.

    \item \textbf{Data Persistence:} Amazon S3 provides durable, highly available object storage for all document artifacts and processed outputs (markdown, images, metadata), partitioned by user ID for multi-tenant isolation. Objects are encrypted with server-side encryption using customer-managed AWS KMS keys. Amazon DynamoDB stores user metadata, chat histories, quiz results, and feedback with automatic replication across availability zones for fault tolerance.
...
\subsubsection{Data Flow: Processing Pipeline}

When a student or instructor uploads a document, the system follows this asynchronous data flow:
...
\subsubsection{Data Flow: Retrieval and Generation}

When a student submits a query via \texttt{POST /api/search}:
...
\textbf{User Authentication:} The system implements JWT (JSON Web Token)-based authentication for all API requests. Users authenticate via the \texttt{/api/auth/login} endpoint using email and password credentials, receiving a signed JWT token with a 24-hour expiration. Each subsequent API request must include the token in the \texttt{Authorization: Bearer <token>} header. Token validation is performed at the API gateway level before requests are routed to backend services.
...
\subsubsection{Data Protection and Encryption}

\textbf{Encryption In-Transit:} All client-to-server and server-to-server communication uses TLS 1.2 or higher. AWS Certificate Manager (ACM) provisions and auto-renews HTTPS certificates for the public domain. Internally, microservice-to-microservice communication within the VPC also uses TLS, enforced through service mesh policies or VPC endpoint configurations, preventing man-in-the-middle attacks on internal traffic.
...
\subsubsection{Data Isolation and Access Control}
...
\subsubsection{Content Safety and Abuse Prevention}
