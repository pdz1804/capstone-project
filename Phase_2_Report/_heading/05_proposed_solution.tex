...
\begin{itemize}
    ...
    \item \textbf{Explainability:} Fine-tuned models encode knowledge in their internal weights, making it impossible to trace where a specific answer came from. RAG, by contrast, retrieves the actual source documents, so you can verify the claim by reading the original text. This transparency is critical for academic integrity.
\end{itemize}
...
\textbf{Architecture Suits an MRAG System} React's architecture is well suited to the heterogeneous outputs produced by our RAG system, including streaming text answers, references to PDF pages, lecture video timestamps, and relevant diagrams.

Its declarative programming model allows the interface to be described in terms of application state rather than manual DOM manipulation. In practice, this enables the system to be organized into distinct, reusable components such as \texttt{ChatWindow}, \texttt{SourceDocViewer}, \texttt{VideoPlayer}, and \texttt{ImageViewer}, each responsible for a specific modality or interaction pattern.

The main application layer coordinates these components and renders them according to the backend response. This separation of concerns is important not only for maintainability but also for future extensibility as new media types or interface behaviors are added.

\textbf{High Performance with the Virtual DOM}

Because the interface must update dynamically as text and media arrive from the backend, rendering efficiency is important. React's Virtual Document Object Model (DOM) maintains an in-memory representation of the interface, computes the minimal set of changes, and reduces expensive direct manipulation of the browser DOM. This helps preserve responsiveness during frequent UI updates.
...
Transitioning to production demands rigorous evaluation of latency, scalability, and Total Cost of Ownership (TCO). Driven by its \textbf{Rust-based engine}, Qdrant demonstrates exceptional performance, maintaining 30-40 millisecond p95 latencies, significantly outperforming Go-based alternatives in tail latency. Economically, Qdrant offers a generous perpetual free tier for prototyping and scales cost-effectively via custom sharding and the Raft consensus protocol, whereas enterprise solutions like Milvus and Vespa carry higher base infrastructure costs.
...
To benchmark the performance of different retrieval architectures, we utilized the \textbf{Microsoft MS MARCO} (Machine Reading Comprehension) dataset\footnote{Microsoft. (2026, June 1). microsoft/ms\_marco. Hugging Face. Retrieved June 24, 2026, from \url{https://huggingface.co/datasets/microsoft/ms_marco}}. This dataset is a standard benchmark for information retrieval tasks.
...
\begin{equation*}
    \text{Score}_{\text{BM25}}(Q, D) = \sum_{t \in Q} \text{IDF}(t) \cdot \dfrac{\text{TF}(t, D) \cdot (k_1 + 1)}{\text{TF}(t, D) + k_1 \cdot \left(1 - b + b \cdot \dfrac{|D|}{\text{avgdl}}\right)}.
\end{equation*}
...
\begin{equation*}
    \text{Score}_{\text{Dense}}(Q, D) = \dfrac{\vec{V}_{Q} \cdot \vec{V}_{D}}{|\vec{V}_{Q}| \cdot |\vec{V}_{D}|}.
\end{equation*}
...
\begin{enumerate}
    \item \textbf{Recall@k:} Measures the percentage of relevant documents successfully retrieved within the top $k$ results, computed as
    \[
    \text{Recall@k} = \dfrac{| \{ \text{Relevant Docs} \} \cap \{ \text{Retrieved Top-k} \} |}{| \{ \text{Total Relevant Docs} \} |}
    \]
    \item \textbf{nDCG@k (Normalized Discounted Cumulative Gain):} This metric evaluates the \textit{ranking quality}. It rewards algorithms that place relevant documents at the top of the list, while penalizing those that rank them lower. The formula is
    \[
    \text{nDCG@k} = \dfrac{DCG@k}{IDCG@k} \quad \text{where} \quad DCG@k = \sum_{i=1}^{k} \dfrac{2^{rel_i} - 1}{\log_2(i+1)}
    \]
\end{enumerate}
...
This section presents the proposed architecture of the BK-MInD MRAG system, organized from a high-level system overview down to specific architectural components for retrieval and security.

\subsection{High-Level Architecture}\label{sec:highlevel_arch}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.75\textwidth]{img/pdz/Report 252 Diagram-High Level Architecture.png}
...
\begin{itemize}
    ...
    \item \textbf{Vector Storage:} Qdrant Cloud is provisioned as an externally managed vector database service, reducing operational overhead while providing reliable multi-tenant vector search with built-in security isolation. Dense text embeddings (\texttt{edu\_text\_chunks}) and image embeddings (\texttt{edu\_image\_pages}) are stored in dedicated Qdrant collections for independent scaling. BM25 sparse indices are maintained locally for efficient keyword retrieval.
\end{itemize}
...
\textit{Text Embedding Pathway}: Structured Markdown files are segmented using \texttt{RecursiveCharacterTextSplitter} (1024-character chunks, 200-character overlap) and encoded into 384-dimensional vectors by \texttt{all-MiniLM-L6-v2}. Dense vectors are indexed in Qdrant's \texttt{edu\_text\_chunks} collection. The retrieval layer supports: (1) BM25 for keyword retrieval, (2) Dense Qdrant search for semantic similarity, and (3) Hybrid fusion combining both via Reciprocal Rank Fusion (RRF).
...
\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{img/pdz/49-security-arch.png}
...
\label{sec:architecture_constraints}
...
\begin{itemize}
    ...
    \item \textbf{Memory Scalability}: \textit{(Resolved in Phase 2 Implementation)} Qdrant Cloud provides managed vector storage with automatic scaling, eliminating the RAM constraints of local index storage. Vector indices are queried remotely via Qdrant's HNSW implementation without requiring local memory.
