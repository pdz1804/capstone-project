...
\begin{equation*}
    z_m = f_m(x_m; \theta_m)
\end{equation*}
where $x_m$ is the raw input of modality $m$, $f_m(\cdot)$ is the corresponding encoder (e.g., Transformer encoder for text, Conformer for audio, ViT for vision), and $\theta_m$ denotes its learnable parameters. An example of multimodal embedding is shown in Figure~\ref{fig:Multimodal embedding}\footnote{The AiEdge Newsletter. (n.d.). How to build a multimodal RAG pipeline with LlamaIndex. Retrieved June 24, 2026, from \url{https://newsletter.theaiedge.io/p/how-to-build-a-multimodal-rag-pipeline-8d6}}.
...
Industry systems also focus on interleaved documents such as PDFs and slides. Nomic's Embed Multimodal 7B embeds mixed text-image document structures for RAG over visually rich files\footnote{Nomic AI. (n.d.). Nomic Embed Multimodal. Retrieved June 24, 2026, from \url{https://www.nomic.ai/blog/posts/nomic-embed-multimodal}}.
...
A MRAG pipeline is illustrated in Figure~\ref{fig:MRAG pipeline}\footnote{The AiEdge Newsletter. (n.d.). How to build a multimodal RAG pipeline with LlamaIndex. Retrieved June 24, 2026, from \url{https://newsletter.theaiedge.io/p/how-to-build-a-multimodal-rag-pipeline-8d6}}.
...
The augmentation process in RAG is illustrated in Figure~\ref{fig:Augmentation}\footnote{Obot AI. (2024, May 21). Understanding RAG: 6 steps of retrieval augmented generation (RAG). Retrieved June 24, 2026, from \url{https://obot.ai/resources/learning-center/retrieval-augmented-generation/}}.
...
Prompt engineering also supports \textbf{structured and reliable outputs}. Schema-based prompts can request JSON, bullet lists, or domain templates, reducing parsing failures in downstream systems\footnote{OpenAI. (2023). GPT-4 technical report. Retrieved June 24, 2026, from \url{https://openai.com/research/gpt-4}}. Function calling extends this idea by constraining outputs to well-defined tool or data schemas.
