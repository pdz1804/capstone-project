...
class MediaProcessor:
    ...
    def _save_media_metadata(self, file_path: Path, results: Dict) -> Path:
        ...
        metadata = {
            ...
            "outputs": {
                "audio_path": str(results.get("audio_path")) if results.get("audio_path") else None,
                "transcript_path": str(results.get("transcript_path")) if results.get("transcript_path") else None,
                "chunks_path": str(results.get("chunks_path")) if results.get("chunks_path") else None,
                "frame_count": len(results.get("frame_paths", [])),
                "frame_paths": [str(p) for p in results.get("frame_paths", [])]
            },
            "video_properties": {
