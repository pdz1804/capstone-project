...
The system implementation requires a robust schema design to manage heterogeneous data including user interactions, processed documents, and high-dimensional vector representations. We utilize a dual-database approach to balance persistence and low-latency retrieval:

\begin{itemize}
    \item \textbf{Amazon DynamoDB}: A fully managed \textbf{NoSQL} database service that provides fast and predictable performance with seamless scalability. In our architecture, DynamoDB serves as the primary store for persistent metadata, user profile information, assessment results, and conversational state (chat sessions/messages). Its schema-less nature allows for flexible data structures while its partitioned key-value access ensures low-latency operations even as the user base scales.
    \item \textbf{Qdrant}: A specialized \textbf{Vector Database} and similarity search engine designed for storing and querying high-dimensional embeddings. Qdrant manages the multi-pathway vector representations of textual chunks and visual document pages, enabling semantic similarity matching and retrieval-augmented generation. It provides optimized search algorithms, filtering capabilities, and distributed indexing to maintain sub-second retrieval performance across deep vector collections.
\end{itemize}
    ...
    \begin{forest}
      for tree={
        ...
        edge path={
          ...
          (!u.east) -- +(5pt,0) |- (.child anchor) \forestoption{edge label};
        },
      }
      [S3
...
\begin{table}[H]
    ...
    \begin{tabular}{llp{8cm}}
        ...
        vote & String & User sentiment (\texttt{like} or \texttt{dislike}). \\
        reason\_code & String & Categorized reason for the feedback (e.g., \texttt{other}, \texttt{incorrect}). \\
        reason\_text & String & Qualitative text provided by the user. \\
