...
class MultimodalDocumentProcessor:
    ...
    DOCLING_SUPPORTED_FORMATS = {
        ...
        'audio': ['.wav', '.mp3', '.m4a', '.flac'],  # InputFormat.AUDIO - ASR pipeline if available
        'video': ['.vtt'],                  # InputFormat.VTT - Video text tracks
        'web': ['.html', '.htm'],           # InputFormat.HTML - Default Docling processing
        'text': ['.md']                     # InputFormat.MD - Default Docling processing
    }
    ...
    def export_processed_document(self, processing_info: Dict[str, Any]) -> Dict[str, str]:
        ...
        try:
            ...
            doc_output_dir = self.output_dir / file_stem
            doc_output_dir.mkdir(parents=True, exist_ok=True)
            
            # Export ONLY to Markdown (main content)
            try:
                markdown_content = doc.export_to_markdown()
                markdown_path = doc_output_dir / f"{file_stem}.md"
            ...
            if self.config.export_images:
                extracted_images = self.extract_images_from_document(doc, file_stem, doc_output_dir, doc)
                if extracted_images:
            ...
            if self.config.export_tables:
                extracted_tables = self.extract_tables_from_document(doc, file_stem, doc_output_dir)
                if extracted_tables:
            ...
            if self.config.export_metadata:
                ...
                metadata_path = doc_output_dir / f"{file_stem}_metadata.json"
            ...
            logger.info(f"Successfully exported {file_stem} to {len(exported_files)} formats")
            
        except Exception as e:
            logger.error(f"Error exporting document {file_stem}: {e}")
