...
def main() -> None:
    ...
    parser.add_argument("--target-invocations-per-instance", type=float, default=10.0)
    ...
    parser.add_argument("--unified-max-concurrent-gpu-ops", type=int, default=10)
    parser.add_argument("--colqwen-max-concurrent-inferences", type=int, default=10)
    parser.add_argument("--wait", action="store_true")
    args = parser.parse_args()

    env = parse_env_pairs(args.env)
    env.setdefault("AWS_REGION", args.region)
    env.setdefault("UNIFIED_MAX_CONCURRENT_GPU_OPS", str(max(1, args.unified_max_concurrent_gpu_ops)))
    env.setdefault("COLQWEN_MAX_CONCURRENT_INFERENCES", str(max(1, args.colqwen_max_concurrent_inferences)))
