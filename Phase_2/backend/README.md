# Unified RAG Pipeline - Backend

A comprehensive, production-ready pipeline that combines **document processing** and **information retrieval** into a complete RAG (Retrieval-Augmented Generation) system with a FastAPI web server.
...
- 🌐 **FastAPI Server** - REST API for file management, processing, indexing, and search
...
```bash
# From backend directory (not api/)
...
| `/api/upload` | POST | Upload one or more files |
| `/api/files` | DELETE | Delete a specific file |
| `/api/files/download` | POST | Download a file |

### **Processing & Indexing**

| Endpoint | Method | Description |
|----------|--------|-------------|
| `/api/status` | GET | Get pipeline status (ready, indexed docs, etc.) |
| `/api/process` | POST | Process uploaded documents through pipeline |
| `/api/index` | POST | Build/rebuild retrieval indexes |

### **Search & Query**

| Endpoint | Method | Description |
|----------|--------|-------------|
| `/api/search` | POST | Search documents with text/image retrieval |
...
The backend also supports direct CLI usage via `run_pipeline.py` for batch processing without the web server.
