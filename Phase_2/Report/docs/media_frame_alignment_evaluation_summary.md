...
$$
\text{WER} = \dfrac{\text{word-level Levenshtein distance(reference, prediction)}}{\max(1, \text{reference word count})}
$$

**Character Error Rate (`char_error_rate`, CER):**

$$
\text{CER} = \dfrac{\text{character-level Levenshtein distance(reference, prediction)}}{\max(1, \text{reference character count})}
$$
...
$$
\min\left(1.0, \dfrac{\sum_{C_i \in \text{top-k}} \text{overlap}(G, C_i)}{g_e - g_s}\right)
$$
