...
Open `http://localhost:5001/docs` for OpenAPI.
...
Optional: `python -m pytest -m unit` (markers in `pytest.ini`). Pass extra pytest args directly, e.g. `python -m pytest tests\services -q`.
...
| Variable | Meaning |
|----------|---------|
| `USE_AWS_SAGEMAKER_INFERENCE=true` | Use **SageMaker Runtime** for ColQwen `embed-query` / `embed-images` (JSON contract matches `Phase_2/code/sagemaker`). |
| `AWS_REGION` | e.g. `us-west-2` |
| `SAGEMAKER_ENDPOINT_NAME` | e.g. `phase2-multimodal-rt` |
