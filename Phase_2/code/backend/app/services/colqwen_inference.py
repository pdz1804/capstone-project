"""ColQwen embeddings: local GPU/CPU or AWS SageMaker endpoint."""
...
class ColQwenInferenceService:
    ...
    def _ensure_local(self) -> Tuple[Any, Any]:
        with self._lock:
            ...
            dtype_str = str(cq.get("dtype", "bfloat16"))
            load_4 = bool(cq.get("load_in_4bit", False))
            load_8 = bool(cq.get("load_in_8bit", False))
            cache_key = f"{model_name}|{dtype_str}|4={int(load_4)}|8={int(load_8)}"

            with ColQwenInferenceService._global_lock:
                cached = ColQwenInferenceService._global_model_cache.get(cache_key)
                if cached is not None:
                    self._model, self._processor = cached
                    logger.info("ColQwen cache hit: reusing loaded model (%s)", model_name)
                    return self._model, self._processor

                self._load_local()
                ColQwenInferenceService._global_model_cache[cache_key] = (self._model, self._processor)
                logger.info("ColQwen cache store: model retained for reuse (%s)", model_name)
            return self._model, self._processor
