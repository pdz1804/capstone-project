...
def process_video(
    ...
    try:
        ...
        # ========================= STEP 3: RAG INDEXING =========================
        if enhanced_chunks_path and Path(enhanced_chunks_path).exists():
            try:
                ...
                chunks = rag_pipeline.load_enhanced_chunks(enhanced_chunks_path)
                ...
                if chunks:
                    ...
                    if success:
                        ...
                        logger.info(f"✓ Indexed {len(chunks)} chunks with hybrid retrieval")
                    else:
    ...
    if result.get("retrieval_ready"):
        ...
        if search_query:
            try:
                ...
                if Path(enhanced_chunks_path).exists():
                    # Load chunks
                    with open(enhanced_chunks_path, 'r') as f:
                        chunks_data = json.load(f)
                    ...
                    frame_metadata_dict = {}
                    if Path(frame_meta_path).exists():
                        try:
